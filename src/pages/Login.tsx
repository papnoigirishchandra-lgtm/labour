...
import { loginUser, loginWithGoogle } from "@/integrations/firebase.auth";
      ...
      try {
        ...
        if (!existingProfile) {
          await supabase.from("profiles").insert({
            ...
            full_name: result.user.displayName || result.user.email?.split("@")[0] || null,
        ...
        console.warn("Could not create profile records:", err);
      }
  ...
  return (
    <div className="min-h-screen bg-hero bg-glow flex items-center justify-center px-4 py-28">
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="glass-strong rounded-2xl p-8 w-full max-w-md relative z-10">
        <h1 className="font-display text-2xl font-bold text-center mb-2">Welcome <span className="gradient-text">Back</span></h1>
        ...
        <button
          ...
          className="w-full glass glass-hover rounded-xl px-6 py-3 text-sm font-medium text-foreground flex items-center justify-center gap-2 transition-all disabled:opacity-50"
        >
